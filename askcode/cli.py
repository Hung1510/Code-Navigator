...
cli.py — the command-line interface.
    ...
    askcode index  <repo> [--full]
    askcode ask    <repo> "question" [--mode] [--no-rerank] [--json]
    askcode search <repo> "question" [--mode] [--no-rerank] [--json]
...
def _load_graph_or_exit(repo: str):
    ...
    if g is None:
        raise SystemExit(f"No call graph found. Run `askcode index {repo}` first.")
    return g
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="askcode", description="Ask your codebase in plain language.")
